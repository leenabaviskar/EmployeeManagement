...
public class AdminAccountServiceImpClass implements AdminAccountService
{
	...
	public void save(AdminAccount admin)
	{
		try 
		{
			...
			PreparedStatement ps=connection.prepareStatement("insert into adminaccount(adminusername, adminemail, adminpassword)values(?,?,?)");
	...
	{ 
		...
		try
		{
			...
			PreparedStatement ps=connection.prepareStatement("select * from adminaccount where adminusername = ? and adminpassword = ?");
